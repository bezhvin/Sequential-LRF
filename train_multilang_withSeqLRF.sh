...
# train_multilang_withSeqLRF.sh is a modification of steps/nnet2/train_multilang2.sh for multi-language training of neural nets.  
# it is changed such that an extra input option low_rank_value can be taken and depending on the number of epochs, factorization will
# be emplyed sequentially. This script also keeps the model before factorization at each steps with _old suffix.
...
stage=-4
low_rank_value=500 # low-rank value
...
if [ $# -lt 6 -o $[$#%2] -ne 0 ]; then
  ...
  echo "  --low-rank-value <#low-rank-value|500>                   # Low-rank value"
...
echo "There will be $num_iters iterations for each language."
numlayers=`nnet-am-info --print-args=false $dir/0/0.mdl | grep num-updatable-components | awk '{print $2}'`
echo " There are $numlayers linear weight layers in the DNN structure."
echo " It is assumed that the final weight layer is already factorized. If not do it first."
echo " Thus, there are $[$numlayers- 3] wight layers left to be factorized. (We skip the input weight layer)"
init_facttime=$[$num_iters/$[$numlayers- 2] ]
facttime=$init_facttime

echo " Factorization occurs every $facttime iteration."
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    layerstep=`echo $((x / facttime))`
    layerID=$[$numlayers-$layerstep]
    
     if [ $((x % facttime)) -eq 0 ] && [ $layerID -ge 1 ]; then # is it factorization time?
     
     echo "=====================>>>> Iteration $x :::::: Factorizing Layer: $layerID ..."
     for lang in $(seq 0 $[$num_lang-1]); do
     (
     cp -r  $dir/$lang/$x.mdl $dir/$lang/${x}_old.mdl
     nnet-am-limit-rank-layer --dim=$low_rank_value --layer=$layerID $dir/$lang/${x}_old.mdl $dir/$lang/$x.mdl
     )&
     done
     wait
